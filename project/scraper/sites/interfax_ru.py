...
def get_news_links_interfax_ru(request=None):
    sitemap_url = Source.objects.get(link="https://www.interfax.ru/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:25]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_interfax_ru(link)
...
def fetch_article_details_interfax_ru(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            for ul_class in article_text_div.find_all(["figure", "script", "iframe", "aside", 'div', 'link', 'meta']):
                ul_class.decompose()
