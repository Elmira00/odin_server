...
@check_source_active("https://sputnik-georgia.ru/")
def get_news_links_sputnik_georgia_ru(request):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    sitemap_url = "https://sputnik-georgia.ru/news/"
    ...
    if response.status_code == 200:
        ...
        website_base_url = "https://sputnik-georgia.ru"
        ...
        for link in links[:20]:
            title,description, content, image_url,gallery_images,news_shared_date = fetch_article_details_sputnik_georgia_ru(link)

            process_article_data(
            source_link="https://sputnik-georgia.ru/",
            link=link,
            title=title,
...
def fetch_article_details_sputnik_georgia_ru(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            if article_text_div.find(['div'], class_=['article__vote', 'vote__content', 'vote__body', 'vote__footer']):
                content = None
            else:
                        
                        
                for block in article_text_div.find_all('div', class_='article__block', attrs={'data-type': 'text'}):
                    inner_text_div = block.find('div', class_='article__text')
                    if inner_text_div:
                        a_tag = inner_text_div.find('a')
                        if a_tag and 'Подписывайтесь на наш канал в Telegram' in a_tag.get_text(strip=True):
                            block.decompose()
                ...
                content = clean_html_withregex("".join(cleaned_blocks))

                soup = BeautifulSoup(content, "html.parser")

                container = soup.body or soup

                clean_soup_tags(soup, container)

                content = str(container)

                content = clean_donya_e_eqtesad_com(content)
