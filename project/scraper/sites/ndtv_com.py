...
@check_source_active("https://www.ndtv.com/")
def get_news_links_ndtv_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.ndtv.com/").rss_link
    ...
    try:
        ...
        for link in links[:20]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_ndtv_com(link, page)

            process_article_data(
                source_link="https://www.ndtv.com/",
                link=link,
                title=title,
...
def fetch_article_details_ndtv_com(url, page: WebPage):
    new_tab = page.new_tab()
    html = None

    try:
        new_tab.get(url, timeout=60)
        new_tab.wait.eles_loaded('div.Art-exp_wr')
        html = new_tab.html
    ...
    if html:
        ...
        date_str = soup.find("meta", attrs={"name": "publish-date"})["content"]
        news_shared_date = datetime.strptime(date_str, "%a, %d %b %Y %H:%M:%S %z")

        #content

        article_text_div = soup.find("div", {'class': 'Art-exp_wr'})
        ...
        if article_text_div:
            ...
            for ul_class in article_text_div.find_all(["figure", 'script']):
                ul_class.decompose()
