# rss: https://theiranpost.com/feed/
# iran
...
def get_news_links_theiranpost_com(request=None):
    sitemap_url = Source.objects.get(link="https://theiranpost.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for item in items:
            ...
            if link and pub_date_raw and rss_description:
                if link not in link_set:
                    dt = parser.parse(pub_date_raw)
                    dt_baku = dt.astimezone(pytz.timezone("Asia/Baku"))                    
                    links.append((link, dt_baku, rss_description))
                    link_set.add(link)

        for link, news_shared_date, rss_description in links[:11]:
            title, description, content, image_url, gallery_images = fetch_article_details_theiranpost_com(link)
            if not description:
                description = rss_description
                
            process_article_data(
                source_link="https://theiranpost.com/",
                link=link,
                title=title,
...
def fetch_article_details_theiranpost_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        if not article_text_div:
            article_text_div = soup.find('div', {'aria-live': 'polite'})

        if article_text_div:

            CLASS_MATCHES = {
                "img": ['img'],
                'div': ['jp-relatedposts', 'post-item-metadata', 'addtoany_share_save_container']
            }
            ...
            for ul_class in article_text_div.find_all(["figure", "script", "iframe", "aside", 'svg', 'style', 'nav']):
                ul_class.decompose()
