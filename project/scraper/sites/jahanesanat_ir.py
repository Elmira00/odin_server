...
def get_news_links_jahanesanat_ir(request=None):
    sitemap_url = Source.objects.get(link="https://jahanesanat.ir/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:10]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_jahanesanat_ir(link)

            process_article_data(
                source_link="https://jahanesanat.ir/",
                link=link,
                title=title,
...
def fetch_article_details_jahanesanat_ir(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find("div", {'id': 'main_ck_editor'})
