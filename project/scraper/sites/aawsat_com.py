...
@check_source_active("https://aawsat.com/")
def get_news_links_aawsat_com(request=None):
    sitemap_url = Source.objects.get(link="https://aawsat.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, rss_image in links[:20]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_aawsat_com(link)
            if not image_url:
                image_url = rss_image
                
            process_article_data(
                source_link="https://aawsat.com/",
                link=link,
                title=title,
...
def fetch_article_details_aawsat_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        if not image_url:
            img_div = soup.find('div', class_='img-field')
            if img_div:
        ...
        news_shared_date = soup.find("meta", {"property": "og:updated_time"})["content"] if soup.find("meta", {"property": "og:updated_time"}) else None
