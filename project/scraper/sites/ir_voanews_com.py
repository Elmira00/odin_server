# rss: https://ir.voanews.com/api/zkup_empmy
# iran
...
def get_news_links_ir_voanews_com(request=None):
    sitemap_url = Source.objects.get(link="https://ir.voanews.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date, rss_image in links[:20]:
            title, description, content, image_url, gallery_images = fetch_article_details_ir_voanews_com(link)
            if not image_url:
                image_url = rss_image

            process_article_data(
                source_link="https://ir.voanews.com/",
                link=link,
                title=title,
...
def fetch_article_details_ir_voanews_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            for ul_class in article_text_div.find_all(["figure", "script", "iframe", "aside", 'br', 'svg']):
                ul_class.decompose()
