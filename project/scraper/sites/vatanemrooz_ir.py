# rss: https://vatanemrooz.ir/fa/rss/allnews
# iran
...
def get_news_links_vatanemrooz_ir(request=None):
    sitemap_url = Source.objects.get(link="https://vatanemrooz.ir/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:50]:
            title, description, content, image_url, gallery_images = fetch_article_details_vatanemrooz_ir(link)

            process_article_data(
                source_link="https://vatanemrooz.ir/",
                link=link,
                title=title,
...
def fetch_article_details_vatanemrooz_ir(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        image_url = image_url if 'logo.png' not in image_url else None

        # content

        article_text_div = soup.find('article', {'class': 'body'})

        if article_text_div:

            gallery_images = extract_gallery_images(article_text_div)
            gallery_images = ['https://vatanemrooz.ir' + img if img.startswith('/') else img for img in gallery_images]
