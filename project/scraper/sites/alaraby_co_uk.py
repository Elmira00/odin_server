...
@check_source_active("https://www.alaraby.co.uk/")
def get_news_links_alaraby_co_uk(request):

    sitemap_url = Source.objects.get(link="https://www.alaraby.co.uk/").rss_link
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:30]:
            title,description, content, image_url,gallery_images = fetch_article_details_alaraby_co_uk(link)

            process_article_data(
            source_link="https://www.alaraby.co.uk/",
            link=link,
            title=title,
...
def fetch_article_details_alaraby_co_uk(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find('div', class_='field field--name-body field--type-text-with-summary field--label-hidden field__item')
        if article_text_div:
