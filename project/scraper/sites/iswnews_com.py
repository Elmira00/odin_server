# rss: https://iswnews.com/category/news/feed/
...
def get_news_links_iswnews_com(request=None):
    sitemap_url = Source.objects.get(link="https://iswnews.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:15]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_iswnews_com(link)

            process_article_data(
                source_link="https://iswnews.com/",
                link=link,
                title=title,
...
def fetch_article_details_iswnews_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find("article", {'class': 'dw-content'})
        ...
        if article_text_div:
            ...
            CLASS_MATCHES = {
                "div": ["content-title", 'dw-thumbnail', 'dw-tags'],
