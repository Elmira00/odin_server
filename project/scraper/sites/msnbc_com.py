# rss: https://feeds.nbcnews.com/msnbc/public/news
# region: US
import requests
...
@check_source_active("https://www.msnbc.com/")
def get_news_links_msnbc_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.msnbc.com/").rss_link
    ...
    if response.status_code == 200:
        ...
        for link in links[:25]:
            title,description, content, image_url,gallery_images,news_shared_date = fetch_article_details_msnbc_com(link)
            
            process_article_data(
                source_link="https://www.msnbc.com/",
                link=link,
                title=title,
...
def fetch_article_details_msnbc_com(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        if not article_text_div:
            article_text_div = soup.find("div", class_="showblog-body__content")
