# rss: https://pakobserver.net/feed/
# Pakistan
...
def get_news_links_pakobserver_net(request=None):
    sitemap_url = Source.objects.get(link="https://pakobserver.net/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for item in items:
            ...
            if link and 'station' not in link and rss_description:
                if link not in link_set:
                    links.append((link, rss_description))
                    link_set.add(link)

        for link, rss_description in links[:10]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_pakobserver_net(link)
            if not description:
                description = rss_description

            process_article_data(
                source_link="https://pakobserver.net/",
                link=link,
                title=title,
...
def fetch_article_details_pakobserver_net(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
