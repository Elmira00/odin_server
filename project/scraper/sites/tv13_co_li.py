...
@check_source_active("https://13tv.co.il/")
def get_news_links_13tv_co_il(request=None):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    sitemap_url = "https://13tv.co.il/news/abroad/"
    ...
    links = []
    link_set = set()
    website_base_url = "https://13tv.co.il"

    news_cards = soup.find_all('a', class_='Poststyles__Thumbnail-sc-jc7y0v-0 ZPjTy')    
    ...
    for link in links[:30]:
        title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_13tv_co_il(link)
        process_article_data(
            source_link="https://13tv.co.il/",
            link=link,
            title=title,
...
def fetch_article_details_13tv_co_il(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        date_str = soup.find("meta", attrs={"name": "publishDate"})
        if date_str:
            date_str = date_str.get("content")
        ...
        article_text_div = soup.find("div", {"class": "Articlestyles__TextContainer-sc-1dwkya0-6"})
