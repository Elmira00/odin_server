...
@check_source_active("https://tejaratnews.com/")
def get_news_links_tejaratnews_com(request):

    sitemap_url = Source.objects.get(link="https://tejaratnews.com/").rss_link
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:40]:
            title,description, content, image_url,gallery_images = fetch_article_details_tejaratnews_com(link)
            
            process_article_data(
            source_link="https://tejaratnews.com/",
            link=link,
            title=title,
...
def fetch_article_details_tejaratnews_com(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
