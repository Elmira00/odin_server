# rss: https://www.walesonline.co.uk/news/wales-news/?service=rss
# region: UK
...
@check_source_active("https://www.walesonline.co.uk/")
def get_news_links_walesonline_co_uk(request=None):
    sitemap_url = Source.objects.get(link="https://www.walesonline.co.uk/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, rss_image in links[:25]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_walesonline_co_uk(link)
            if not image_url:
                image_url = rss_image

            process_article_data(
                source_link="https://www.walesonline.co.uk/",
                link=link,
                title=title,
...
def fetch_article_details_walesonline_co_uk(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        if not article_text_div:
            article_text_div = soup.find("article", {"class": re.compile(r'LiveEvents_article*')})
        if not article_text_div:
        ...
        if article_text_div:
            ...
            CLASS_MATCHES = {
                "div": ["Byline_byline-container__K_Owl", 'BoxStyles_box-container__Qk3WH',
                        ...
                        "UpdateButton_wrapper__QnGQJ", "LiveEventKeyEvents_key-events-wrapper__5CNY7", 
                        "LiveEventFeed_section-entries__jcAex", "ImageEmbed_image-embed__0T8WX"],
            ...
            for ul_class in article_text_div.find_all(["figure", "script", "iframe", "aside", "a", 'h1', 'h2', 'span', "nav", "header", "section"]):
                ul_class.decompose()
