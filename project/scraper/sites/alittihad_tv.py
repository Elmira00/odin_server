# rss: https://alittihad.tv/rss.xml/
...
@check_source_active("https://www.alittihad.tv/")
def get_news_links_alittihad_tv(request=None):
    sitemap_url = Source.objects.get(link="https://www.alittihad.tv/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:10]:
            title, description, content, image_url, gallery_images = fetch_article_details_alittihad_tv(link)

            process_article_data(
                source_link="https://www.alittihad.tv/",
                link=link,
                title=title,
...
def fetch_article_details_alittihad_tv(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
