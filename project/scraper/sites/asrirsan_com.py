# rss: https://www.asriran.com/fa/rss/allnews
# iran
...
def get_news_links_asriran_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.asriran.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:20]:
            title, description, content, image_url, gallery_images = fetch_article_details_asriran_com(link)

            process_article_data(
                source_link="https://www.asriran.com/",
                link=link,
                title=title,
...
def fetch_article_details_asriran_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            CLASS_MATCHES = {
                "div": ['news_controll', 'social_nets', 'latest_news2', 'news_end', 'border-left', 'row'],
                'img': ['image_btn']
            }
            ...
            gallery_images = ["https://www.asriran.com" + img if img.startswith("/") else img for img in gallery_images]

            for ul_class in article_text_div.find_all("ul", class_="gallery_attach_list"):
                ul_class.decompose()

            for ul_class in article_text_div.find_all("div", id="MV_afterBody"):
                ul_class.decompose()
