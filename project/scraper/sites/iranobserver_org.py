# rss: https://www.iranobserver.org/feed/
# iran
...
def get_news_links_iranobserver_org(request=None):
    sitemap_url = Source.objects.get(link="https://www.iranobserver.org/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:10]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_iranobserver_com(link)

            process_article_data(
                source_link="https://www.iranobserver.org/",
                link=link,
                title=title,
...
def fetch_article_details_iranobserver_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    try:
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            CLASS_MATCHES = {
                "div": ['mh-social-top', 'mh-social-bottom'],
            }
