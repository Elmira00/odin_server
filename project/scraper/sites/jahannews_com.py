# rss: https://cdn.jahannews.com/feeds/rssb5.-er48r6--4qhfle2m.puirug.r.xml
# iran
...
def get_news_links_jahannews_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.jahannews.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for item in items:
            ...
            if link and pub_date_raw:
                if link not in link_set and '/news/' in link:
        ...
        for link, news_shared_date in links[:10]:
            title, description, content, image_url, gallery_images = fetch_article_details_jahannews_com(link)

            process_article_data(
                source_link="https://www.jahannews.com/",
                link=link,
                title=title,
...
def fetch_article_details_jahannews_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find_all('article')[1]
