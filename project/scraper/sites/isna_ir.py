...
@check_source_active("https://www.isna.ir/")
def get_news_links_isna_ir(request):
    
    sitemap_url = Source.objects.get(link="https://www.isna.ir/").rss_link
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:30]:
            title,description, content, image_url,gallery_images = fetch_article_details_isna_ir(link)
            
            process_article_data(
                source_link="https://www.isna.ir/",
                link=link,
                title=title,
...
def fetch_article_details_isna_ir(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find('div', class_='item-body content-full-news')
        if article_text_div:
