# rss: https://eghtesademeli.com/feed
# iran
...
def get_news_links_eghtesademeli_com(request=None):
    sitemap_url = Source.objects.get(link="https://eghtesademeli.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:10]:
            title, description, content, image_url, gallery_images = fetch_article_details_eghtesademeli_com(link)

            process_article_data(
                source_link="https://eghtesademeli.com/",
                link=link,
                title=title,
...
def fetch_article_details_eghtesademeli_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find('div', {'class': 'the_content_body'})
