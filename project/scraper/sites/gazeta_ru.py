...
def get_news_links_gazeta_ru(request=None):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    sitemap_url = "https://www.gazeta.ru/news/"
    ...
    links = []
    link_set = set()
    website_base_url = "https://www.gazeta.ru"

    news_cards = soup.find_all('a', class_='b_ear m_techlisting')    
    ...
    for link in links[:30]:
        title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_gazeta_ru(link)
        process_article_data(
            source_link="https://www.gazeta.ru/",
            link=link,
            title=title,
...
def fetch_article_details_gazeta_ru(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find("div", {"class": "b_article-text"})
