# rss: https://rcs.mako.co.il/rss/news-world.xml
# Israel
...
@check_source_active("https://www.mako.co.il/")
def get_news_links_mako_co_il(request=None):
    sitemap_url = Source.objects.get(link="https://www.mako.co.il/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for item in items:
            ...
            rss_description = item.find("shortDescription").text.strip() if item.find("shortDescription") else None
        ...
        for link, rss_image, rss_title, rss_description, rss_date in links[:20]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_mako_co_il(link)
            if not content:
                continue
            if not image_url:
                image_url = rss_image
            ...
            process_article_data(
                source_link="https://www.mako.co.il/",
                link=link,
                title=title,
...
def fetch_article_details_mako_co_il(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find("div", {'class': 'ArticleBodyWrapper_news__GrFd4'})
        ...
        if article_text_div:
            ...
            CLASS_MATCHES = {
                'div': ['OutbrainRender_root__q3TgT', 'ImageRender_root__MCZJD', 'AdRender_root__8uP5m', 'Tags_root__3QVvm']
            }
