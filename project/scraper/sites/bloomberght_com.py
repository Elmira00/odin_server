...
@check_source_active("https://www.bloomberght.com/")
def get_news_links_bloomberght_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.bloomberght.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:20]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_bloomberght_com(link)

            process_article_data(
                source_link="https://www.bloomberght.com/",
                link=link,
                title=title,
...
def fetch_article_details_bloomberght_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find("div", {'class': 'article-wrapper'})
