# rss: https://www.mizanonline.ir/en/rss/allnews
# region: Iran
...
def get_news_links_mizanonline_ir(request=None):
    sitemap_url = Source.objects.get(link="https://www.mizanonline.ir/en").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:51]:
            title, description, content, image_url, gallery_images = fetch_article_details_mizanonline_ir(link)

            process_article_data(
                source_link="https://www.mizanonline.ir/en",
                link=link,
                title=title,
...
def fetch_article_details_mizanonline_ir(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        image_url = 'https://mizanonline.ir' + scrape_meta_url(soup)
