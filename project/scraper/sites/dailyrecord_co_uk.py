# rss: https://www.dailyrecord.co.uk/?service=rss
# region: UK
...
@check_source_active("https://www.dailyrecord.co.uk/")
def get_news_links_dailyrecord_co_uk(request=None):
    sitemap_url = Source.objects.get(link="https://www.dailyrecord.co.uk/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, rss_image in links[:25]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_dailyrecord_co_uk(link)
            if image_url is None:
                image_url = rss_image

            process_article_data(
                source_link="https://www.dailyrecord.co.uk/",
                link=link,
                title=title,
...
def fetch_article_details_dailyrecord_co_uk(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = article_text_div or soup.find("div", {'class': 'live-event-lead-entry'})
