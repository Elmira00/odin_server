...
@check_source_active("https://www.cbsnews.com/")
def get_news_links_cbsnews_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.cbsnews.com/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:30]:
            title, description, content, image_url, gallery_images, news_shared_date = (
                fetch_article_details_cbsnews_com(link)
            )

            process_article_data(
                source_link="https://www.cbsnews.com/",
                link=link,
                title=title,
...
def fetch_article_details_cbsnews_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find("section", class_="content__body")

        if article_text_div:
            for tag in article_text_div.find_all(["aside", "style"]):
                tag.decompose()

            gallery_images = extract_gallery_images(article_text_div)

            CLASS_MATCHES = {
                "div": ["ad-wrapper", "content-author"],
                "ul": ["content__tags"],
                "p": ["content__meta--contributors"],
            }
            ...
            for tag in article_text_div.find_all(["figure"]):
                tag.decompose()
