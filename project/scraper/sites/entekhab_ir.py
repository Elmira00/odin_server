...
@check_source_active("https://www.entekhab.ir/")
def get_news_links_entekhab_ir(request):
    
    sitemap_url = Source.objects.get(link="https://www.entekhab.ir/").rss_link
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:20]:
            title,description, content, image_url,gallery_images = fetch_article_details_entekhab_ir(link)
            
            process_article_data(
                source_link="https://www.entekhab.ir/",
                link=link,
                title=title,
...
def fetch_article_details_entekhab_ir(url):
    try:
    ...
    if response.status_code == 200:
        ...
        #content     
        article_text_div = soup.find('div', class_='body col-xs-36')
        if article_text_div:
            ...
            unwanted_div_classes = ["YN yn-article-display-card yn-desktop "]
