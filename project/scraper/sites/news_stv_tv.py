# rss: https://news.stv.tv/section/world/feed
# region: UK
...
def get_news_links_news_stv_tv(request=None):
    sitemap_url = Source.objects.get(link="https://news.stv.tv/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:50]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_news_stv_tv(link)

            process_article_data(
                source_link="https://news.stv.tv/",
                link=link,
                title=title,
...
def fetch_article_details_news_stv_tv(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
