...
@check_source_active("https://donya-e-eqtesad.com/")
def get_news_links_donya_e_eqtesad_com(request):
    
    sitemap_url = Source.objects.get(link="https://donya-e-eqtesad.com/").rss_link
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:30]:
            title,description, content, image_url,gallery_images = fetch_article_details_donya_e_eqtesad_com(link)
            
            process_article_data(
                source_link="https://donya-e-eqtesad.com/",
                link=link,
                title=title,
...
def fetch_article_details_donya_e_eqtesad_com(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            unwanted_div_classes = ["inline-box-theme1 inline-box-id","noprint"]
