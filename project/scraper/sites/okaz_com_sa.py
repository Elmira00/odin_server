# rss: https://www.okaz.com.sa/rssFeed/0
...
@check_source_active("https://www.okaz.com.sa/")
def get_news_links_okaz_com_sa(request=None):
    sitemap_url = Source.objects.get(link="https://www.okaz.com.sa/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, rss_image in links[:20]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_okaz_com_sa(link)
            if not image_url:
                image_url = rss_image

            process_article_data(
                source_link="https://www.okaz.com.sa/",
                link=link,
                title=title,
...
def fetch_article_details_okaz_com_sa(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
