...
@check_source_active("https://www.wam.ae/ar/home/main")
def get_news_links_wam_ae(request=None):
    sitemap_url = "https://www.wam.ae/ar/list/latest-news"
    website_base_url = "https://www.wam.ae"
    ...
    try:
        ...
        page.wait.eles_loaded('a.post-title')

        soup = BeautifulSoup(page.html, "html.parser")

        if soup:
            items = soup.find_all('a', class_='post-title')
        ...
        for link in links[:20]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_wam_ae(link, page)

            process_article_data(
                source_link="https://www.wam.ae/ar/home/main",
                link=link,
                title=title,
...
def fetch_article_details_wam_ae(url, page: WebPage):
    ...
    if html:
        ...
        tag = soup.find("small", class_="text-muted")
        if tag:
        ...
        article_text_div = soup.find("article", class_="content")
