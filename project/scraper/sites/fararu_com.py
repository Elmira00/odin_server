...
@check_source_active("https://fararu.com/")
def get_news_links_fararu_com(request):
     
    sitemap_url = Source.objects.get(link="https://fararu.com/").rss_link
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:20]:
            title,description, content, image_url,gallery_images = fetch_article_details_fararu_com(link)
            
            process_article_data(
            source_link="https://fararu.com/",
            link=link,
            title=title,
...
def fetch_article_details_fararu_com(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            for svg_tag in article_text_div.find_all('svg'):
                svg_tag.decompose()
