...
@check_source_active("https://www.aysor.am/en")
def get_news_links_aysor_am(request=None):
    sitemap_url = "https://www.aysor.am/en/news"
    website_base_url = "https://www.aysor.am"
    ...
    for link in links[:15]:
        title, description, content, image_url, gallery_images, news_shared_date = (
            fetch_article_details_aysor_am(link)
        )
        process_article_data(
            source_link="https://www.aysor.am/en",
            link=link,
            title=title,
...
def fetch_article_details_aysor_am(url):
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        context = browser.new_context(
            user_agent=(
