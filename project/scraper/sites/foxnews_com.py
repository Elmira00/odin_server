...
@check_source_active("https://www.foxnews.com/")
def get_news_links_foxnews_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.foxnews.com/").rss_link
    ...
    if response.status_code == 200:
        ...
        for item in items:
            ...
            if link and rss_description and rss_title and rss_content and rss_image and rss_date:
                if link not in link_set:
                    rss_date = parser.parse(rss_date)
                    links.append((link, rss_description, rss_title, rss_content, rss_image, rss_date))
                    link_set.add(link)
                
        for link, rss_description, rss_title, rss_content, rss_image, rss_date in links[:20]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_foxnews_com(link)
            if not description:
            ...
            process_article_data(
                source_link="https://www.foxnews.com/",
                link=link,
                title=title,
...
def fetch_article_details_foxnews_com(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        if not description:
            description = soup.find('h2', {'class': 'sub-headline'}).get_text(strip=True) if soup.find('h2', {'class': 'sub-headline'}) else None
        ...
        if article_text_div:     
            ...
            for a_tag in article_text_div.find_all('a', rel='noopener'):
                a_tag.decompose()

            for fc_tag in article_text_div.find_all('figcaption'):
                fc_tag.decompose()

            for fc_tag in article_text_div.find_all('picture'):
                fc_tag.decompose()
