# rss: https://mikroskopmedia.com/en/feed/
# Azerbaijan
...
def get_news_links_mikroskopmedia_com(request=None):
    sitemap_url = Source.objects.get(link="https://mikroskopmedia.com/en/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:10]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_mikroskopmedia_com(link)

            process_article_data(
                source_link="https://mikroskopmedia.com/en/",
                link=link,
                title=title,
...
def fetch_article_details_mikroskopmedia_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            for ul_class in article_text_div.find_all("a", {'rel': 'noreferrer'}):
                ul_class.decompose()
