...
@check_source_active("https://iz.ru/")
def get_news_links_izru(request):
    sitemap_url = Source.objects.get(link="https://iz.ru/").rss_link
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:45]:
            title,description, content, image_url,gallery_images = fetch_article_details_izru(link)
            
            process_article_data(
            source_link="https://iz.ru/",
            link=link,
            title=title,
...
def fetch_article_details_izru(url):
    headers  = {"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}
    ...
    if response.status_code == 200:
        ...
        content_element = soup.find('div', class_='text-article__inside')
        if content_element:
            news_content = content_element.find('div', itemprop='articleBody')
            if news_content:
                
                for div_class in news_content.find_all('div', class_='more_style_one'):
                    div_class.decompose()
        ...
        image_element = soup.find('div', class_='big_photo__img')
        if image_element:
            img_tag = image_element.find('img')
            if img_tag:
                image_url = img_tag.get('data-src', img_tag.get('src', 'No Image Found'))
