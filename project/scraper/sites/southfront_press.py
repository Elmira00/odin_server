# rss: https://southfront.press/feed/
...
@check_source_active("https://southfront.press/")
def get_news_links_southfront_press(request=None):
    sitemap_url = Source.objects.get(link="https://southfront.press/").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link in links[:15]:
            title, description, content, image_url, gallery_images, news_shared_date = fetch_article_details_southfront_press(link)

            process_article_data(
                source_link="https://southfront.press/",
                link=link,
                title=title,
...
def fetch_article_details_southfront_press(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        article_text_div = soup.find('div', {'style': 'text-align: justify;'})
        ...
        if article_text_div:
            ...
            for ul_class in article_text_div.find_all(["figure", "script", "iframe", "aside", "blockquote", "ul", "hr"]):
                ul_class.decompose()

            for ul_class in article_text_div.find_all('strong')[-1]:
                ul_class.decompose()
