# rss: https://www.fardanews.com/feeds/
# region: Iran
...
@check_source_active("https://www.fardanews.com/")
def get_news_links_fardanews_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.fardanews.com/").rss_link
    headers = {
        ...
        for item in items:
            ...
            if link:
                if link not in link_set:
                    links.append((link, news_shared_date))
                    link_set.add(link)

        for link, news_shared_date in links[:30]:
            title, description, content, image_url, gallery_images = fetch_article_details_fardanews_com(link)

            process_article_data(
                source_link="https://www.fardanews.com/",
                link=link,
                title=title,
...
def fetch_article_details_fardanews_com(url):
    headers = {
    ...
    if response.status_code == 200:
        ...
        if article_text_div:
            ...
            CLASS_MATCHES = {
                "div": ["inline-news-box", 'noprint', 'inline-news-box2', 'others_known'
                        'sticky_zxc_bottom', 'quad_news', 'sticky_notify2'],
            }
