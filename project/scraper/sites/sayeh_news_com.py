# rss: https://www.sayeh-news.com/fa/rss
# iran
...
def get_news_links_sayeh_news_com(request=None):
    sitemap_url = Source.objects.get(link="https://www.sayeh-news.com/fa/online").rss_link
    headers = {
    ...
    if response.status_code == 200:
        ...
        for link, news_shared_date in links[:35]:
            title, description, content, image_url, gallery_images = fetch_article_details_sayeh_news_com(link)

            process_article_data(
                source_link="https://www.sayeh-news.com/fa/online",
                link=link,
                title=title,
...
def fetch_article_details_sayeh_news_com(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"
    }
    ...
    if response.status_code == 200:
        ...
        title = soup.find('meta', {'name': 'og:title'})['content'] if soup.find('meta', {'name': 'og:title'}) else None
        description = soup.find('meta', {'name': 'description'})['content'] if soup.find('meta', {'name': 'description'}) else None
        image_url = soup.find('meta', {'name': 'og:image'})['content'] if soup.find('meta', {'name': 'og:image'}) else None

        # content

        article_text_div = soup.find('div', {'class': 'detail-body'})
