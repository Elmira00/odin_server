...
class BaseAutoHealer:
    ...
    def __init__(self, source_id, test_urls):
    ...
    def fetch_soup(self, url):
        try:
            print(f"[DEBUG] Subprocess (Terminal CURL) request to: {url}")
            ...
            cmd = [
                ...
                '-A', 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/115.0' 
            ]
            
            result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=15)
            
            html_text = result.stdout
            print(f"[DEBUG] Terminal Curl HTML length: {len(html_text)} chars")
            ...
            if len(html_text) > 500:
                ...
                if not title_text or len(title_text) < 5:
                    print("[DEBUG] Page <title> is missing or too short!")
                    print("[DEBUG] Possible Anti-Bot page detected. Rejected.")
                    return None
       
                print(f"[DEBUG] Page title: {title_text}")
                return soup
            else:
                print(f"[DEBUG] Terminal curl returned empty HTML.")
                print(f"[DEBUG] CURL ERROR (stderr): {result.stderr.strip()}")
                return None
...
class ImageAutoHealer(BaseAutoHealer):
    ...
    def get_all_media_candidates(self, soup):
        if is_mathrubhumi_domain(soup):
            print("[DEBUG] Mathrubhumi Helper activated.")
            return get_mathrubhumi_media_candidates(soup) 
            
        raw_candidates = []
        
        if is_interfax_domain(soup):
            print("[DEBUG] Interfax Helper activated.")
        ...
        for elem in raw_candidates:
            ...
            for parent in elem.parents:
                ...
                if parent.get('class'):
                    ...
                    if any(c in p_classes for c in ['sidebar', 'right-column', 'widgets', 'timeline',
                     'related', 'feed', 'stream', 'comments', 'recommend', 'slider', 'carousel', 
                     'promo','podcast', 'audio', 'briefing','banner','blocker','advert', 'sponsor', 'adbox', 'insideind']):
                        parent_has_bad_class = True
                        break
            ...
            if elem.name == 'img':
                ...
                safe_meta_str = meta_str.replace('uploads', '').replace('threads', '')
    ...
    def find_image_by_tf_idf(self, soup):
        ...
        try:
            ...
            print(f"[DEBUG] Best image TF-IDF score: {similarities[best_idx]}")
            
            if similarities[best_idx] > 0.01: 
                return candidates[best_idx]
        except Exception as e:
            print(f"[DEBUG] TF-IDF Error: {e}")
    ...
    def run_diagnostics(self):
        ...
        for index, url in enumerate(self.test_urls, 1):
            print(f"\n[DEBUG] TEST URL {index}/{len(self.test_urls)}: {url} ---")
            soup = self.fetch_soup(url)
            if not soup:
                print(f"[DEBUG] URL {index} could not be opened or blocked by Anti-Bot.")
                continue

            if is_mathrubhumi_domain(soup):
                print("[DEBUG] Mathrubhumi specific cleaner activated.")
            ...
            h1_src = target_node_h1.get('src') or target_node_h1.get('data-src') if target_node_h1 else 'None'
            print(f"[DEBUG] 1. H1 Proximity: {target_node_h1.name if target_node_h1 else 'None'} -> {h1_src}")
            
            tfidf_src = target_node_tfidf.get('src') or target_node_tfidf.get('data-src') if target_node_tfidf else 'None'
            print(f"[DEBUG] 2. TF-IDF: {target_node_tfidf.name if target_node_tfidf else 'None'} -> {tfidf_src}")
            
            heur_src = target_node_heur.get('src') or target_node_heur.get('data-src') if target_node_heur else 'None'
            print(f"[DEBUG] 3. Heuristic: {target_node_heur.name if target_node_heur else 'None'} -> {heur_src}")
            print("------------------------------------------\n")
            ...
            if winner_tag:
                ...
                print(f"[DEBUG] Found CSS Selector for URL {index}: {new_selector}")
                generated_selectors.append(new_selector)
            else:
                print(f"[DEBUG] No suitable image found for URL {index}.")

        valid_selectors = [sel for sel in generated_selectors if sel]
        
        if not valid_selectors:
            print("[DEBUG] No image found across any URL. DB will not be updated.")
            return None
        ...
        if best_count >= 2 and (best_count / total_valid) > 0.5:
            print(f"[DEBUG] PERFECT RULE FOUND! {best_count}/{total_valid} confirmed the selector.")
            print(f"[DEBUG] FINAL SELECTOR: {best_selector}")
            return {"resolution_type": "css_selector", "rule": best_selector}
        elif total_valid == 1 and len(self.test_urls) <= 2:
            print("[DEBUG] Insufficient data, but accepting the only found selector.")
            return {"resolution_type": "css_selector", "rule": best_selector}
        else:
            print(f"[DEBUG] RULE REJECTED! No stability. Best selector found only {best_count} times.")
            print(f"[DEBUG] Vote distribution: {dict(selector_counts)}")
            return None

class SharedDateAutoHealer(BaseAutoHealer):
    def __init__(self, source_id, test_urls):
        super().__init__(source_id, test_urls)
        self.meta_tags = [
        ...
        for script in scripts:
            ...
            try:
                ...
                for item in items:
                    ...
                    if item.get("datePublished") or item.get("dateModified") or item.get("uploadDate"):
                        return True
            except Exception:
                continue
        return False
    ...
    def find_date_node(self, soup):
        for tag in soup.find_all('time'):
            if tag.has_attr('datetime') or self.is_valid_date_format(tag.get_text()):
                return tag
    ...
    def run_diagnostics(self):
        ...
        for index, url in enumerate(self.test_urls, 1):
            ...
            # if self.check_json_ld_date(soup):
            #     print("[DEBUG] Structured date found in JSON-LD.")
            #     json_ld_count += 1
            #     continue 
            ...
            if date_node:
                ...
                if url_date_match:
                    print(f"[DEBUG] Date found in URL: {url_date_match.group(1)}")
                    url_patterns.append(r'(20\d{2}[-/]\d{1,2}[-/]\d{1,2})')
                else:
                    print("[DEBUG] No date/time format found in URL.")

        total_urls = len(self.test_urls)
        
        # if json_ld_count >= 2 or (total_urls == 1 and json_ld_count == 1):
        #     print("\n[DEBUG] PERFECT RULE: JSON-LD consensus achieved.")
        #     return {"resolution_type": "json_ld", "rule": "datePublished"}
        ...
        if generated_selectors:
            ...
            if best_count >= 2 and (best_count / len(generated_selectors)) > 0.5:
                print("\n[DEBUG] PERFECT RULE: DOM CSS Selector consensus achieved.")
                return {"resolution_type": "css_selector", "rule": best_selector}
        ...
        if url_patterns:
            ...
            if best_count >= 2 or (total_urls == 1 and best_count == 1):
                print("\n[DEBUG] PERFECT RULE: URL Regex consensus achieved.")
                return {"resolution_type": "url_regex", "rule": best_pattern}

        print("\n[DEBUG] RULE REJECTED! No stability or date missing.")
        return None
...
class TitleAutoHealer(BaseAutoHealer):
    ...
    def check_json_ld_title(self, soup):
        scripts = soup.find_all("script", type="application/ld+json")
        for script in scripts:
            ...
            try:
                ...
                for item in items:
                    if not isinstance(item, dict): continue
                    if item.get("headline") or item.get("alternativeHeadline") or (item.get("@type") == "NewsArticle" and item.get("name")):
                        return True
            except Exception:
                continue
        return False
    ...
    def run_diagnostics(self):
        ...
        for index, url in enumerate(self.test_urls, 1):
            ...
            # meta_value, successful_mapping = self.check_meta_tags(soup, self.meta_tags)
            # if meta_value and len(meta_value.strip()) > 3:
            #     return {"resolution_type": "meta", "rule": successful_mapping}

            # if self.check_json_ld_title(soup):
            #     return {"resolution_type": "json_ld", "rule": "headline"}

            target_node = self.find_title(soup)
            if target_node:
                new_selector = self.generate_css_selector(target_node)
                if new_selector and new_selector.count('nth-of-type') <= 2 and len(new_selector) < 150:
                    generated_selectors.append(new_selector)
        ...
        if generated_selectors:
            ...
            if best_count >= 2 or (len(self.test_urls) == 1 and best_count == 1):
                return {"resolution_type": "css_selector", "rule": best_selector}
...
class DescriptionAutoHealer(BaseAutoHealer):
    ...
    def check_json_ld_description(self, soup):
        scripts = soup.find_all("script", type="application/ld+json")
        for script in scripts:
            if not script.string: continue
            try:
                ...
                for item in items:
                    if not isinstance(item, dict): continue
                    if item.get("description") or item.get("abstract"):
                        return True
            except Exception:
                continue
        return False
    ...
    def find_description(self, soup):
        ...
        desc_elements = soup.find_all(['div', 'p', 'span', 'h2', 'h3'], 
                                      class_=lambda x: x and any(v in str(x).lower() for v in desc_classes))
    ...
    def run_diagnostics(self):
        ...
        for index, url in enumerate(self.test_urls, 1):
            ...
            # if self.check_json_ld_description(soup):
            #     return {"resolution_type": "json_ld", "rule": "description"}
...
class ContentAutoHealer(BaseAutoHealer):
    ...
    def find_content(self, soup):
        ...
        if data_target:
            if not data_target.find_parent(['nav', 'footer', 'aside', 'header']):
                return data_target
        h1_tag = soup.find('h1')
        search_scope = soup
        if h1_tag:
        ...
        content_pattern = re.compile(r'article-body|post-content|entry-content|story-body|article-text|page-content|main-content|article_content|content', re.IGNORECASE)
        ...
        if content_elements:
            ...
            for element in content_elements:
                ...
                if any(bw in element_classes or bw in parent_classes for bw in bad_words):
                    continue

                paragraphs = element.find_all('p')
                if len(paragraphs) >= 2 or len(element.text.strip()) > 200:
                    return element
        ...
        for div in all_divs:
            if div.find_parent(['nav', 'footer', 'aside', 'header']):
                continue
        ...
        if best_block and max_p_count >= 2:
            return best_block
    ...
    def run_diagnostics(self):
        ...
        for index, url in enumerate(self.test_urls, 1):
            ...
            # if self.check_json_ld(soup, ['articleBody']):
            #     return {"resolution_type": "json_ld", "rule": "articleBody"}
