#project/management/auto_healer/resolvers_adapter.py
"""
...
class DescriptionAutoHealer(_DescriptionAutoHealer):
    def get_base_candidates(self, soup, field, url):
        ...
        # SIGNAL 1: Meta tags (og:description, twitter:description) are explicitly designed 
        # for page-level excerpts. They are far more reliable than JSON-LD, which often 
        # leaks the global site tagline. Meta gets top priority.
        meta_value, meta_rule = self.check_meta_tags(soup, self.meta_tags)
        if meta_value and len(meta_value.strip()) > 5:
            candidates.append(Candidate(
                ...
                source="base_meta",
                value=meta_value,
                prior_confidence=0.8,  # Increased to beat JSON-LD
            ))

        # SIGNAL 2: JSON-LD description acts as a fallback if OpenGraph is completely missing.
        ...
        if json_ld_desc and len(json_ld_desc.strip()) > 15:
            candidates.append(Candidate(
                ...
                source="base_json_ld",
                value=json_ld_desc,
                prior_confidence=0.6,  # Decreased so meta takes priority
            ))

        # SIGNAL 3: DOM search is the last resort.
