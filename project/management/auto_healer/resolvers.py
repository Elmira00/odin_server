...
class SharedDateAutoHealer(BaseAutoHealer):
    def __init__(self, source_id, test_urls, shared_soups):
        ...
        self.meta_tags = [
            ...
            {"tag": "meta", "attrs": {"itemprop": "datePublished"}},  # NEW: Catch standard Microdata
            {"tag": "meta", "attrs": {"itemprop": "dateModified"}},   # NEW
            {"tag": "meta", "attrs": {"itemprop": "uploadDate"}},     # NEW
    ...
    def is_valid_date_format(self, text):
        ...
        textual_pattern = re.compile(r'\b(?:\d{1,2}\s+[a-zA-Zа-яА-ЯəşçğıöüƏŞÇĞIÖÜ\u10D0-\u10FA]+|[a-zA-Zа-яА-ЯəşçğıöüƏŞÇĞIÖÜ\u10D0-\u10FA]+\s+\d{1,2})(?:,?\s+20\d{2})?\b', re.IGNORECASE)
        time_pattern = re.compile(r'\b(?:[01]\d|2[0-3]):[0-5]\d\b')
        return bool(numeric_pattern.search(text) or textual_pattern.search(text) or time_pattern.search(text))
    ...
    def find_date_node(self, soup):
        ...
        # Only match visible DOM nodes for CSS selectors.
        # Meta tags are now safely handled in self.meta_tags (Tier 2).
        itemprop_tags = soup.find_all(['span', 'div', 'p', 'time'], attrs={"itemprop": re.compile(r'datePublished|dateModified|uploadDate', re.I)})
        if itemprop_tags:
        ...
        h1_tag = soup.find('h1') or soup.find(['h2', 'h3'], class_=lambda x: x and 'title' in x.lower())
        ...
        for tag in soup.find_all(['span', 'div', 'p', 'li', 'a']):
            ...
            if tag.find_parent(class_=re.compile(r'related|recommend|teaser|widget|footer|sidebar|nav|menu', re.I)):
                continue

            if self.is_valid_date_format(text):
                score = 0
                
                # SIGNAL 1: Penalize Links, but forgive explicit semantic date links
                if tag.name == 'a' or tag.find_parent('a'):
                    a_node = tag if tag.name == 'a' else tag.find_parent('a')
                    a_class = " ".join(a_node.get("class", [])).lower()
                    a_href = str(a_node.get("href", "")).lower()
                    
                    if "time" in a_class or "date" in a_class or "date" in a_href:
                        score -= 10  # Mild penalty: recognized as a semantic archive link
                    else:
                        score -= 100 # Heavy penalty: likely a related article link
                    
                # SIGNAL 2: Penalize List Items
                if tag.name == 'li' or tag.find_parent('li'):
                    score -= 30
                ...
                # SIGNAL 4: Proximity to H1
                if h1_tag:
                    try:
                        dist = self.calculate_dom_distance(h1_tag, tag)
                        if dist < 15:
                            score += (15 - dist) * 2  
                    except ValueError:
                        pass
                ...
                # SIGNAL 5: Purity
                date_chars = sum(1 for c in text if c.isdigit() or c in ' :.-/,|()+')
                purity = date_chars / len(text)
                score += purity * 20

                # SIGNAL 6: Timezone Context
                if re.search(r'\b(?:UTC|GMT|Z)\b|[+-]\d{2}:?\d{2}\b', text, re.IGNORECASE):
                    score += 40
...
class DescriptionAutoHealer(BaseAutoHealer):
    ...
    def run_diagnostics(self):
        ...
        # Tier 1: Standard meta description (og:description, etc.) is the most reliable 
        # signal for page-level excerpts. Checked across all fetched samples.
        meta_rule = self.resolve_from_value_tier(
            soups,
            lambda s: self.check_meta_tags(s, self.meta_tags),
            min_len=6,
        ...
        # Tier 2: JSON-LD description as a fallback, checked across all fetched samples.
        # Demoted to Tier 2 to prevent global site taglines (like Yoast SEO) from overriding OpenGraph.
        json_ld_rule = self.resolve_from_value_tier(
            soups,
            lambda s: (self.get_json_ld_value(s, self.json_ld_keys), {"source": "json_ld", "keys": self.json_ld_keys}),
            min_len=15,
...
class ContentAutoHealer(BaseAutoHealer):
    ...
    NEGATIVE_HINTS = re.compile(r'gallery|comment|footnote|foot|nav|sidebar|sponsor|share|social|widget|advert|banner|related|promo|masthead|byline|caption|teaser|header|timestamp|dateline|meta\b', re.IGNORECASE)
    ...
    def _find_by_semantic_class(self, search_scope):
        ...
        content_pattern = re.compile(r'content-inner|article-body|post-content|entry-content|story-body|article-text|page-content|main-content|article_content|article__body|\bcontent\b', re.IGNORECASE)
