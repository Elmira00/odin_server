...
#             t = str(t or "").lower()
#             return any(k in t for k in ("article", "newsarticle", "blogposting", "webpage","block","main","content"))
...
#     POSITIVE_HINTS=re.compile(r'post|thumbnail|image|figure|',re.IGNORECASE)
...
#             if elem.name == 'img':
#                 if any(w in src_lower for w in ['no-image', 'placeholder', 'author', 'yazar', 'avatar', 'profile','post-image']):
#                     continue
...
#     def run_diagnostics(self):
#         # Tier 1: OpenGraph / Twitter / itemprop meta — cheapest and most reliable,
#         # and previously defined but never actually used.
#         # for url, soup in self.iter_cleaned_soups():
...
#         #     break  # meta tags should be identical in shape across a source; one check is enough to decide the *type*

#         # Tier 1b: JSON-LD image (only used to sanity-check DOM guesses below, since it's
#         # a value not a selector — kept out of the return path to preserve the selector-based schema).
...
#         for url, soup in self.iter_cleaned_soups():
#             # Tier 1: JSON-LD structured date — most reliable, no selector guessing needed.
#             json_ld_date = self.get_json_ld_value(soup, self.json_ld_keys)
#             if json_ld_date:
#                 return {"resolution_type": "meta", "rule": {"source": "json_ld", "keys": self.json_ld_keys}}

#             # Tier 2: standard meta tags.
#             meta_value, successful_mapping = self.check_meta_tags(soup, self.meta_tags)
#             if meta_value:
#                 return {"resolution_type": "meta", "rule": successful_mapping}

#             # Tier 3: DOM search, <time datetime> preferred.
...
#         for url, soup in self.iter_cleaned_soups():
#             # Tier 1: JSON-LD headline — this is what search engines themselves trust,
#             # so it's a strong ground-truth signal even when we still fall back to a
#             # DOM selector (e.g. to cross-check candidate H1s against it below).
...
#     NOISE_WRAPPER_HINTS = re.compile(r'overlay|popup|modal|cookie|banner|newsletter|related|recommend|comments', re.IGNORECASE)
#     POSITIVE_HINTS = re.compile(r'container|body|article|content|entry|main|post|story|text|article|txt|row|entry', re.IGNORECASE)
#     NEGATIVE_HINTS = re.compile(r'|reklam|comment|footnote|foot|nav|sidebar|sponsor|share|social|widget|advert|banner|related|promo|masthead|byline|caption|teaser|header', re.IGNORECASE)
...
#             if self.NOISE_WRAPPER_HINTS.search(element_classes) or self.NOISE_WRAPPER_HINTS.search(parent_classes):
#                 continue
...
#     def _validate_content_node(self, node):
#         if node is None:
#             return False
#         classes = " ".join(node.get('class', [])).lower()
#         if self.NOISE_WRAPPER_HINTS.search(classes):
#             return False
#         if node.find_parent(['nav', 'footer', 'aside', 'header']):
#             return False
...
#         # Long real articles can still have a handful of inline links, but a
#         # block that's mostly link text (nav rail, related-articles list) is
#         # never the article body regardless of how much total text it has.
#         if link_density > 0.5 and text_len < 1000:
#             return False

#         return True

#     def find_content_by_density(self, search_scope):
...
#             if self._validate_content_node(node):
#                 return node
...
#         node = self.find_content_by_density(search_scope)
#         if self._validate_content_node(node):
#             return node
